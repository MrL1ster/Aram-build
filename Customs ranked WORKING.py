...
class Player:
    def __init__(self, name, role=None, hero=None, rank=None, stats=None):
        ...
        self.stats = stats if stats else {'wins': 0, 'losses': 0}
...
def display_heroes_with_numbers(heroes_list):
...
def pick_and_ban(players, heroes, bans_per_team=1):
    ...
    if result == "1":
        for player in team1_players:
            update_stats(player, "win")
        for player in team2_players:
            update_stats(player, "loss")
    else:
        for player in team1_players:
            update_stats(player, "loss")
        for player in team2_players:
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            if players:
                print("\nPlayer Stats:")
                print("{:<20} {:<10} {:<10} {:<10} {:<25} {:<10}".format("Name", "Rank", "Wins", "Losses", "Most Played Hero", "Win rate"))
                print("-" * 100)
                for player in players:
                    print("{:<20} {:<10.2f} {:<10} {:<10} {:<25} {:<10.2f}%".format(player.name, player.rank, player.stats['wins'], player.stats['losses'], player.most_selected_hero(), player.win_rate()))
                print("-" * 100)
            else:
            ...
            team1, team2 = pick_and_ban(players, heroes, bans_per_team=1)
            ...
            if result == 1:
                ...
                display_teams(team1, team2, False)

              # Reset available_heroes for each custom pick and ban game
              ...
              # ban logic
                ...
                for i in range(bans_per_team):
                    for team in [team1, team2]:
                      team_num = 1 if team == team1 else 2
                      player = team[i]
                      print(f"\nTeam {team_num} bans by {player.name}:")
                      hero = input("Enter the hero to ban: ")
                      while hero not in available_heroes:
                          print("Invalid hero name or hero not available. Please enter a valid hero:")
                          hero = input("Enter the hero to ban: ")
                      available_heroes.remove(hero)
                      player.increment_hero_ban_count(hero)

                    print(f"\nBan round {i+1} complete.")
            ...
            # Picking logic
                for i in range(len(team1)):
                    ...
                    # Commented out the line below to stop displaying available heroes when picking
                    # display_available_heroes(available_heroes)  

                        hero = input(f"Enter the hero for player {i + 1}: ")
                        while hero not in available_heroes:
                            print("Invalid hero name. Please enter a valid hero:")
                            hero = input(f"Enter the hero for player {i + 1}: ")
                        available_heroes.remove(hero)
